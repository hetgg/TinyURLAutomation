...
data "aws_iam_policy_document" "lambda-iam-policy" {
  ...
  statement{
    ...
    resources  = [ "${aws_s3_bucket.ece592-week12-patelhe.arn}/*"
	]
}
...
# Now that weve defined the contents of our policy let's create the
# actual resource.
resource "aws_iam_policy" "lambda-iam-policy" {
  name = "week12-lambda-iam-policy"
  path = "/"
